import Pieces
import time,copy,random,operator
#import numpy as np
...
class Column:
    ...
    def __init__(self, pos):
        self.position = pos
        self.col = ['xx','xx','xx','xx','xx','xx','xx','xx']
        self.possiblePieces = ["King","Queen","Bishop","Knight","Rook","Pawn"]
...
##        self.recordBoard()
    ...
    def updateAllPieces(self):
        updateValids = operator.methodcaller('updateValidMoves')
        map(updateValids,self.pieces)
#        for piece in self.pieces:
#            piece.updateValidMoves()
    def updateAllPieces2(self):
        #for testing if move will put me in check
        updateValids = operator.methodcaller('updateValidMoves2')
        map(updateValids,self.pieces)
#        for piece in self.pieces:
#            piece.updateValidMoves2()
    def updateWhitePieces(self):
        #for testing if move will put me in check
        updateValids = operator.methodcaller('updateValidMoves2')
        map(updateValids,self.remainingWhitePieces)
#        for piece in self.remainingWhitePieces:
#            piece.updateValidMoves2()
    def updateBlackPieces(self):
        #for testing if move will put me in check
        updateValids = operator.methodcaller('updateValidMoves2')
        map(updateValids,self.remainingBlackPieces)
...
#            piece.updateValidMoves2()
    ...
    ##                            self.checkRemainingBlackPieces()
                            ...
                            self.rows[checkcoord[0]].setRowPos(checkcoord[1], "xx")
                            self.cols[checkcoord[1]].setColPos(checkcoord[0], "xx")
                            

                        self.rows[startcoord[0]].setRowPos(startcoord[1], "xx")
                        self.rows[endcoord[0]].setRowPos(endcoord[1], piece)
                        self.cols[startcoord[1]].setColPos(startcoord[0], "xx")
                        ...
                        if piece == "King" and start == "E1" and end == "C1":
                            ...
                            self.rows[startcoord2[0]].setRowPos(startcoord2[1], "xx")
                            self.rows[endcoord2[0]].setRowPos(endcoord2[1], piece2)
                            self.cols[startcoord2[1]].setColPos(startcoord2[0], "xx")
...
##                            self.checkRemainingBlackPieces()
    ...
    ##                            self.checkRemainingWhitePieces()
                            ...
                            self.pieces.remove(piece21)
                            piece21.setPosition("X")
                            self.remainingWhitePieces.remove(piece21)
                            self.rows[checkcoord[0]].setRowPos(checkcoord[1], "xx")
                            self.cols[checkcoord[1]].setColPos(checkcoord[0], "xx")
                        self.rows[startcoord[0]].setRowPos(startcoord[1], "xx")
                        self.rows[endcoord[0]].setRowPos(endcoord[1], piece)
...
##                            self.checkRemainingWhitePieces()
    ...
    def checkWhitePossibleMoves(self, spot, shouldPrint=True):
        spot = spot.upper()
...
##            print "here2"
            ...
            if shouldPrint:
                print piece
            piece.updateValidMoves()
            t = piece.getValidMoves()
            if shouldPrint:
                print t
            return (str(piece),t)
    def checkBlackPossibleMoves(self, spot, shouldPrint=True):
        spot = spot.upper()
        piece = self.getPieceAt(spot)        
        if piece != "Piece":
##            print "here1"
...
##        updateValids = operator.methodcaller('updateValidMoves')
##        map(updateValids,self.remainingWhitePieces)
##        getValids = operator.methodcaller('getValidMoves')
##        validMoves = map(getValids,self.remainingWhitePieces)
##        validMovesLength = np.array(map(len,validMoves))
##        return (True in (validMovesLength>0))
...
##        map(updateValids,self.remainingBlackPieces)
##        getValids = operator.methodcaller('getValidMoves')
##        validMoves = map(getValids,self.remainingBlackPieces)
##        validMovesLength = np.array(map(len,validMoves))
##        return (True in (validMovesLength>0))
...
##        return val
    def isEndGame(self):
        return self.ended
    def removePieceAt(self,spot):
        piece = self.getPieceAt(spot)
        self.pieces.remove(piece)
        piece.setPosition("X")
        try:
            self.remainingWhitePieces.remove(piece)
        except ValueError:
            self.remainingBlackPieces.remove(piece)
    def __str__(self):
        s = "  A  B  C  D  E  F  G  H \n"
        i = 9
