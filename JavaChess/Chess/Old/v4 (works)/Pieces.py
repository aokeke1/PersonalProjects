...
class Piece(object):
    ...
    def setPosition(self, position):
        if type(position)== tuple and position[0] in xrange(8) and position[1] in xrange(8):
            r = 8-position[0]
...
##            self.setPosition(self, position)
    def getThreats(self):
        """
            return a list of pieces that are a threat to this piece
        """
        threats = []
        spot = self.getPosition()
        #get a list of all opponent pieces
        if self.getColor() == 'White':
            allOpponentPieces = self.board.checkRemainingBlackPieces()
        else:
            allOpponentPieces = self.board.checkRemainingWhitePieces()
        
        for  piece in allOpponentPieces:
            #update all of their moves
            #if my position is in their move list, add to list
            if spot in piece.getValidMoves():
                threats.append(piece)
        #return list of threats
        return threats
        
    def getThreatenedPieces(self):
        """
            return a list of pieces that this piece threatens
        """
        threatened = []
        if self.getColor() == 'White':
            allOpponentPieces = self.board.checkRemainingBlackPieces()
            ...
            allOpponentPieces = self.board.checkRemainingWhitePieces()
        #check my possible moves
        myMoves = self.getValidMoves()
        #check if there is an opponent piece on any of the squares I can move on
        #if so, add that piece to the list of threatened pieces
        for piece in allOpponentPieces:
            if piece.getPosition() in myMoves:
...
##                print place, "removed. will be check"
        ...
        if not(self.inCheck()):
            if self.color == "White":
                if self.canQueenSideCastle() and self.validMoves.count("C1")==0:
                    self.validMoves.append("C1")
                if self.canKingSideCastle() and self.validMoves.count("G1")==0:
                    self.validMoves.append("G1")
            if self.color == "Black":
                if self.canQueenSideCastle() and self.validMoves.count("C8")==0:
                    self.validMoves.append("C8")
                if self.canKingSideCastle() and self.validMoves.count("G8")==0:
...
##            pause = raw_input("pause1")
            ...
            if coord2[0] in xrange(8) and coord2[1] in xrange(8):
                ...
                #en passant
                elif piece21 == "Pawn" and self.position[1] == "5":
                    if piece21.getColor() != self.color and piece21.getLastPosition()[1]=="7":
                        self.validMoves.append(coordToSquare(coord2))                        
            if coord3[0] in xrange(8) and coord3[1] in xrange(8):
                ...
                #en passant
                elif piece31 == "Pawn" and self.position[1] == "5":
                    if piece31.getColor() != self.color and piece31.getLastPosition()[1]=="7":
                        self.validMoves.append(coordToSquare(coord3))
            if coord1[0] in xrange(8) and coord1[1] in xrange(8):
            ...
            if coord2[0] in xrange(8) and coord2[1] in xrange(8):
                ...
                if piece2 == "Piece":
                    if piece2.getColor() != self.color:
                        self.validMoves.append(coordToSquare(coord2))
                #en passant
                elif piece21 == "Pawn" and self.position[1] == "4":
            ...
            if coord3[0] in xrange(8) and coord3[1] in xrange(8):
                ...
                if piece3 == "Piece":
                    if piece3.getColor() != self.color:
                        self.validMoves.append(coordToSquare(coord3))
                #en passant
                elif piece31 == "Pawn" and self.position[1] == "4":
...
def coordToSquare(coord):
    ...
    if coord[0] in xrange(8) and coord[1] in xrange(8):
        row = 8-coord[0]
