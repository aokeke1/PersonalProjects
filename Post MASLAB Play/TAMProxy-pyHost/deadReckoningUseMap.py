...
#        self.cap.set(4,self.webcamHeight)
        ...
        self.targets = A2.solve3(self.mapFile)
        self.target = (self.targets[0][0],self.targets[0][1],"x")
...
#                    self.target=(0,0,90)
                if self.state == 0:
                    ...
                    if result:
                        self.targetIndex += 1
                        try:
                            self.target = (self.targets[self.targetIndex][0],self.targets[self.targetIndex][1],"x")
                        except IndexError:
                            self.state = -1
#                        self.part1=False
