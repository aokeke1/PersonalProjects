...
#                self.targInd += self.driveToPos(desiredPos)
    ...
    def trackBlocks(self):
        if self.ret:
            self.frame2 = copy.copy(self.frame)
            b,g,r = cv2.split(self.frame2) 
            self.frame2[((r>(self.rg*g)) & (r>(self.rb*b)))] = [0,0,255]
            self.frame2[((g>(self.gr*r)) & (g>(self.gb*b)))] = [0,255,0]
            self.frame2[~(((g>(self.gr*r)) & (g>(self.gb*b)))|((r>(self.rg*g)) & (r>(self.rb*b))))] = [0,0,0]
            
        
        height, width, channels = self.frame2.shape
        contours = self.filter_contours(self.frame2,height,width,ratio = 0.01)
        self.targetLocked = False
        if len(contours)==0:
            self.bias = 0
            ...
            print ("block seen")
            targetBlock = contours[0]
            cv2.drawContours(self.frame, contours, 0, (255,255,0), 3)
            cv2.drawContours(self.frame, [targetBlock], 0, (255,0,255), 3)
            M = cv2.moments(targetBlock)
            try:
                cx = int(M['m10']/M['m00'])
                cy = int(M['m01']/M['m00'])
